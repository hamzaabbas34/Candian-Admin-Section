...
const Products: React.FC = () => {
	...
	// Handle view in front toggle
	const handleViewInFrontToggle = async (
		productId: string,
		currentValue: boolean
	) => {
		try {
			const newValue = !currentValue;
			await api.patch(`/products/${productId}/viewInfront`, {
				viewInfront: newValue,
			});
			...
			setProducts((prevProducts) =>
				prevProducts.map((product) =>
					product._id === productId
						? { ...product, viewInfront: newValue }
						: product
				)
			);

			toast.success(
				`Product ${newValue ? "shown" : "hidden"} in front successfully!`
			);
		} catch (error) {
			console.error("Failed to update view in front:", error);
			toast.error("Failed to update product visibility");
		}
	};

	// Bulk view in front toggle
	const handleBulkViewInFrontToggle = async (value: boolean) => {
		...
		try {
			const updatePromises = selectedProducts.map((productId) =>
				api.patch(`/products/${productId}/viewInfront`, {
					viewInfront: value,
				})
			);
			...
			console.error("Failed to bulk update view in front:", error);
			toast.error("Failed to update products visibility");
		}
	};
	...
	// };
const handlePublish = async (release: Release) => {
	const isCurrentlyPublished = release.isPublished;

	const result = await Swal.fire({
		icon: isCurrentlyPublished ? "warning" : "info",
		title: isCurrentlyPublished ? "Unpublish Version" : "Publish Version",
		text: isCurrentlyPublished
			? `This will unpublish "${release.year} ${release.versionName}". Continue?`
			: `This will publish "${release.year} ${release.versionName}" and unpublish any currently published version. Continue?`,
		showCancelButton: true,
		confirmButtonText: isCurrentlyPublished ? "Yes, Unpublish" : "Yes, Publish",
		cancelButtonText: "Cancel",
		confirmButtonColor: isCurrentlyPublished ? "#ef4444" : "#10b981",
	});
	...
	<Button
		...
		className={`gap-1 ${
			release.isPublished ? "bg-red-500 hover:bg-red-600" : "bg-green-500 hover:bg-green-600"
		}`}
	>
						...
						<div className="flex items-center justify-between">
							...
							<div className="flex gap-2">
								<Button
									size="sm"
									variant="outline"
									onClick={() => handleBulkViewInFrontToggle(true)}
									className="gap-1">
								...
								<Button
									size="sm"
									variant="outline"
									onClick={() => handleBulkViewInFrontToggle(false)}
									className="gap-1">
							...
							<Table>
								<TableHeader>
									<TableRow>
										...
										<TableHead>View In Front</TableHead>
									...
									{products.map((product) => (
										<TableRow key={product._id}>
											...
											<TableCell>
												<Button
													size="sm"
													variant={product.viewInfront ? "default" : "outline"}
													onClick={() =>
														handleViewInFrontToggle(
															product._id,
															product.viewInfront || false
														)
													}
													className="gap-1">
													{product.viewInfront ? (
